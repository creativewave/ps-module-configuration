...
dir := $(notdir $(CURDIR))
dist_files := $(wildcard LICENSE src/*)
...
lint: ## Detect coding style errors (pass FIX=1 to fix them).
	vendor/bin/php-cs-fixer fix --stop-on-violation -v $(if $(FIX),-n -q,--dry-run --diff)
...
publish: $(dir).zip $(dir).tar.gz ## Publish new release on Github.
	...
	&& upload_url=`curl \
		...
		-d '{"tag_name": "'$$version'"}' \
		https://api.github.com/repos/creativewave/$(dir)/releases \
